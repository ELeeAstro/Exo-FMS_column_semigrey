...
contains
  ...
  subroutine sw_DISORT(nlay, nlev, tau_e, mu_z, Finc, ssa, gg, sw_up, sw_down, sw_net, asr)
    implicit none
    ...
    if ((mu_z(nlev) > 0.0_dp) .and. (Finc > 0.0_dp)) then
      ...
      do i = 1, nlay
        ggg(i) = real(gg(i))
        ssalb(i) = real(ssa(i))
        dtauc(i) = real((tau_e(i+1) - tau_e(i)))
      end do
      ...
      call call_DISORT(nlay,Te_0,ggg,ssalb,dtauc,nlev,utau,planck,wvnmlo,wvnmhi,Tint,fbeam,umu0,sw_net_d(:),olr_g)

...
contains
  ...
  subroutine lw_disort(nlay, nlev, Tl, pl, pe, tau_e, ssa, gg, Tint, lw_up, lw_down, lw_net, olr)
    implicit none
    ...
    do i = 1, nlay
      ggg(i) = real(gg(i),sp)
      ssalb(i) = real(ssa(i),sp)
      dtauc(i) = real((tau_e(i+1) - tau_e(i)),sp)
     end do

    do i = 1, nlev
      utau(i) = real(tau_e(i),sp)
    end do

    call call_DISORT(nlay,Te_0,ggg,ssalb,dtauc,nlev,utau,planck,wvnmlo,wvnmhi,real(Tint),fbeam,umu0,lw_net(:),olr)

...
contains
  ...
  subroutine sw_SDA(nlay, nlev, tau_e, mu_z, Finc, ssa, gg, a_surf, sw_up, sw_down, sw_net, asr)
    implicit none
    ...
    if ((mu_z(nlev) > 0.0_dp) .and. (Finc > 0.0_dp)) then
      ...
      call sw_fs_SDA(nlay, nlev, Finc, mu_z(:), tau_e(:), ssa(:), gg(:), a_surf, sw_down(:), sw_up(:))
